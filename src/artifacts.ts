...
const FRONTMATTER = /^---\r?\n([\s\S]*?)\r?\n---/;
const STATUS_VALUES: ArtifactStatus[] = [
  "draft",
  "active",
  "superseded",
  "deprecated",
...
export async function discoverArtifacts(
  ...
  cwd: string,
): Promise<ArtifactDiscovery> {
  const artifactsByPath = new Map<string, GovernedArtifact>();
  const ruleMatchCounts: ArtifactDiscovery["ruleMatchCounts"] = [];
...
export async function readArtifactMetadata(
  ...
  if (
    ...
    STATUS_VALUES.includes(raw.status as ArtifactStatus)
  ) {
    metadata.status = raw.status as ArtifactStatus;
  }
...
export function buildArtifactFindings(
  config: SpecGovConfig,
  ...
  for (const artifact of discovery.artifacts) {
    if (config.rules.require_lifecycle_status && !artifact.status) {
      findings.push({
        ...
        file: artifact.path,
        suggestion:
          "Add YAML frontmatter with status: active, draft, superseded, deprecated, or archived.",
      });
    }

    if (
      config.rules.require_owner_for_active_specs &&
    ...
    if (artifact.status === "superseded" && !artifact.supersededBy) {
      findings.push({
        ...
        file: artifact.path,
        suggestion:
          "Add superseded_by metadata pointing to the replacement artifact.",
      });
    }
    ...
    if (artifact.lastVerified && config.rules.stale_after_days > 0) {
      ...
      if (ageDays !== undefined && ageDays > config.rules.stale_after_days) {
        findings.push({
          ...
          file: artifact.path,
          suggestion:
            "Review the artifact and update last_verified if it still matches reality.",
        });
      }
    }
  }
...
export function artifactPathsMatching(
...
function daysBetween(dateText: string, now: Date): number | undefined {
