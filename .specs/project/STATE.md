...
- npm `1.0.0-rc.3` publication requires the repository's npm Trusted Publisher to authorize `.github/workflows/release.yml`.
...
- Superseded v0.1 active-tree specs, example manifests, and unused documentation assets were removed; their history remains available in Git.
- Repository path confinement, bounded semantic execution, packed-package smoke tests, and a deterministic 500-change-set performance budget are locally verified for RC3.
- External npm RC, GitHub prerelease, external Action, and stable release evidence remain pending.
