...
export async function analyzeRepository(options = {}) {
    ...
    if (options.semantic || effective.semantic.enabled) {
        try {
            ...
            const fail = effective.mode === "strict" &&
                effective.semantic.failure_policy === "fail";
            findings.push(semanticFailure(error.message, fail));
        }
        report = makeReport(effective.mode, graph, changedFiles, findings);
    }
    return report;
}
function makeReport(mode, graph, changedFiles, findings) {
    const errors = findings.filter((f) => f.severity === "error").length, warnings = findings.filter((f) => f.severity === "warning").length, infos = findings.filter((f) => f.severity === "info").length;
    const status = errors || (mode === "strict" && warnings)
        ? "fail"
        : warnings || infos
            ? "warn"
            : "pass";
...
function semanticFailure(message, fail) {
    return {
